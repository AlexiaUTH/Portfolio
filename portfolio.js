/* Popup Image Web*/
function openWeb1() {
  document.getElementById("Web1").style.display = "block";
}
function openWeb2() {
  document.getElementById("Web2").style.display = "block";
}
function openWeb3() {
  document.getElementById("Web3").style.display = "block";
}
function openWeb4() {
  document.getElementById("Web4").style.display = "block";
}
function openWeb5() {
  document.getElementById("Web5").style.display = "block";
}
function openWeb6() {
  document.getElementById("Web6").style.display = "block";
}
function openWeb7() {
  document.getElementById("Web7").style.display = "block";
}
function openWeb8() {
  document.getElementById("Web8").style.display = "block";
}
function openWeb9() {
  document.getElementById("Web9").style.display = "block";
}
function openWeb10() {
  document.getElementById("Web10").style.display = "block";
}

/* Popup Image Graphique*/
function openGraph1() {
  document.getElementById("Graph1").style.display = "block";
}
function openGraph2() {
  document.getElementById("Graph2").style.display = "block";
}
function openGraph3() {
  document.getElementById("Graph3").style.display = "block";
}
function openGraph4() {
  document.getElementById("Graph4").style.display = "block";
}
function openGraph5() {
  document.getElementById("Graph5").style.display = "block";
}
function openGraph6() {
  document.getElementById("Graph6").style.display = "block";
}
function openGraph7() {
  document.getElementById("Graph7").style.display = "block";
}
function openGraph8() {
  document.getElementById("Graph8").style.display = "block";
}
function openGraph9() {
  document.getElementById("Graph9").style.display = "block";
}
function openGraph10() {
  document.getElementById("Graph10").style.display = "block";
}

/* Close Popup Image */


// function closePop_up() {
//   var elms = document.querySelectorAll("#Web1","#Web2");
//   elms.style.display = "none";
// }

function closePop_upGraph() {
  document.getElementById("Graph1").style.display = "none";
  // document.getElementById("Graph2").style.display = "none";
  document.getElementById("Graph3").style.display = "none";
  document.getElementById("Graph4").style.display = "none";
  document.getElementById("Graph5").style.display = "none";
  document.getElementById("Graph6").style.display = "none";
  document.getElementById("Graph7").style.display = "none";
  document.getElementById("Graph8").style.display = "none";
  document.getElementById("Graph9").style.display = "none";
  document.getElementById("Graph10").style.display = "none";
}

function closePop_upWeb() {
  document.getElementById("Web1").style.display = "none";
  // document.getElementById("Web2").style.display = "none";
  document.getElementById("Web3").style.display = "none";
  document.getElementById("Web4").style.display = "none";
  document.getElementById("Web5").style.display = "none";
  document.getElementById("Web6").style.display = "none";
  document.getElementById("Web7").style.display = "none";
  document.getElementById("Web8").style.display = "none";
  document.getElementById("Web9").style.display = "none";
  document.getElementById("Web10").style.display = "none";

}
